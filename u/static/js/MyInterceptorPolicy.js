...
var MyInterceptorPolicy = draw2d.policy.canvas.DropInterceptorPolicy.extend({
    ...
    delegateTarget: function(requestingFigure, connectTarget)
    {
        ...
        //
        if(requestingFigure instanceof Wav && connectTarget instanceof draw2d.Connection){
            if (requestingFigure.getConnections().getSize() == 0)
            {
                return connectTarget;
            }
        }
        if(requestingFigure instanceof Branch && connectTarget instanceof draw2d.Connection){
            if (requestingFigure.classLabel.getConnections().getSize() == 0)
            {
                return connectTarget;
            }
        }
